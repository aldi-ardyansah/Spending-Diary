<?php
	...
	if ($POST_Username !== $Session_Username) {
		$SQL_Query = mysqli_prepare($sql_connection, "SELECT Username FROM tb_a_accounts WHERE Username = ?");
		mysqli_stmt_bind_param($SQL_Query, "s", $POST_Username);
	...
	if ($POST_Current_Password !== "" || $POST_New_Password !== "" || $POST_New_Password_Confirmation !== "") {
		...
		$SQL_Query = mysqli_prepare($sql_connection, "SELECT Password FROM tb_a_accounts WHERE Username = ?");
		mysqli_stmt_bind_param($SQL_Query, "s", $Session_Username);
		mysqli_stmt_execute($SQL_Query);
		$SQL_Result = mysqli_stmt_get_result($SQL_Query);
		$SQL_Fetch = mysqli_fetch_array($SQL_Result);

		if (!$SQL_Fetch || !password_verify($POST_Current_Password, $SQL_Fetch["Password"])) {
			$_SESSION["Account_Failed"] = "Password";
			header("Location: account.php");
			exit;
		}
		...
		$SQL_Query = mysqli_prepare($sql_connection, "UPDATE tb_a_accounts SET Name = ?, Username = ?, Password = ? WHERE Username = ?");
		mysqli_stmt_bind_param($SQL_Query, "ssss", $POST_Name, $POST_Username, $POST_Password_Hash, $Session_Username);
	} else {
		$SQL_Query = mysqli_prepare($sql_connection, "UPDATE tb_a_accounts SET Name = ?, Username = ? WHERE Username = ?");
		mysqli_stmt_bind_param($SQL_Query, "sss", $POST_Name, $POST_Username, $Session_Username);
	}
